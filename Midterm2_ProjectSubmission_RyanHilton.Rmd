...
yhat.nn = predict(nn.gpa,newdata=test.data.sc[,c(1:13)])
...
```{r}
neurons = 8
nn.gpa = neuralnet(Term.GPA~.-Persistence.NextYear, data=train.data.sc, hidden=neurons, linear.output=TRUE, err.fct="sse")
```
...
pr.nn <- pr.nn$net.result*(max(full.data$Term.GPA)-min(full.data$Term.GPA))+min(full.data$Term.GPA)   
test.cv.r <- (test.data.sc$Term.GPA)*(max(full.data$Term.GPA)-min(full.data$Term.GPA))+min(full.data$Term.GPA)   
mse.nn <- (sum((test.cv.r - pr.nn)^2)/(nrow(test.data.sc)-2))
cat("Neural Network Test MSE: ", mse.nn)
```


```{r}
a.table = matrix(c(tree.mse, tree.mseR, bag.mse, bag.mseR, forest.mse, forest.mseR, boost.mse, boost.mseR), ncol = 1)
rownames(a.table) = c("Full Model","Reduced Model","Full Model","Reduced Model","Full Model","Reduced Model","Full Model","Reduced Model")
colnames(a.table) = c("Test MSE")

kable(a.table, caption = "Test MSE for Best Tree and Neural Network Models") %>%
  kable_styling(latex_options = "hold_position") %>%
  pack_rows("Tree Models", 1,2) %>%
  pack_rows("Bagging Models", 3,4) %>%
  pack_rows("Random Forests Models", 5,6) %>%
  pack_rows("Boosting Models", 7,8)
```
...
```{r}
set.seed(99)
bag.year=randomForest(Persistence.NextYear~., data=train.data, mtry=16, importance=TRUE)
bag.year

bag.yearR=randomForest(Persistence.NextYear~., data=r.train.data, mtry=13, importance=TRUE)
bag.yearR
```
...
```{r}
neurons = 8
nn.gpa = neuralnet(Term.GPA~Race_Ethc_Visa+Gender+HSGPA+SAT_Total+Entry_Term+N.Ws+N.DFs+N.As+N.PassedCourse+Perc.Pass+Perc.Withd+N.GraduateCourse+FullTimeStudent+N.RegisteredCourse+N.CourseTaken+Perc.PassedEnrolledCourse, data=train.data.sc, hidden=neurons, threshold = 0.2, stepmax = 1e+05, linear.output=TRUE, err.fct="sse")
```
...
mse.nns <- c()
for(neurons in (1:k)) {
  nn.gpa = neuralnet(Term.GPA~Race_Ethc_Visa+Gender+HSGPA+SAT_Total+Entry_Term+N.Ws+N.DFs+N.As+N.PassedCourse+Perc.Pass+Perc.Withd+N.GraduateCourse+FullTimeStudent+N.RegisteredCourse+N.CourseTaken+Perc.PassedEnrolledCourse, data = train.data.sc, hidden = neurons, threshold = 0.4, stepmax = 1e+05, linear.output = TRUE, err.fct = "sse", rep = 10)
...
for(neurons in (1:k)) {
  for(neurons2 in (1:k2)) {
    ...
    nn.gpa = neuralnet(Term.GPA~Race_Ethc_Visa+Gender+HSGPA+SAT_Total+Entry_Term+N.Ws+N.DFs+N.As+N.PassedCourse+Perc.Pass+Perc.Withd+N.GraduateCourse+FullTimeStudent+N.RegisteredCourse+N.CourseTaken+Perc.PassedEnrolledCourse, data = train.data.sc, hidden = c(neurons,neurons2), threshold = 0.5, stepmax = 1e+05, linear.output = TRUE, err.fct = "sse", rep = 5)
...
#png('TwoLayerNN_RegTest.png', res=100)
...
df2 = df[order(df$Test.MSE),]
df2[(df2$Test.MSE < 1.005),]
```
